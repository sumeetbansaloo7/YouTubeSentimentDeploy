...
model = None
vect = None
with open("./app/router/model.pkl", 'rb') as f:
    model = load(f)
with open("./app/router/vect.pkl", 'rb') as v:
    vect = load(v)
...
async def get_Sentiment(vid: VideoID):
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail="Video ID incorrect. Please request again with proper video ID.")
    print(len(comments))
    # print(type(comments))
